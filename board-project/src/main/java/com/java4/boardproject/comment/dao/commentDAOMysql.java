...
public class commentDAOMysql {
	...
	public List<Comment> getParents(int boardId, int start){
		return jdbcTemplate.query("select a.*, b.name from comments a join users b on a.user_id=b.id where a.board_id=? and a.comment_id is null order by a.id desc limit ?, 5",
