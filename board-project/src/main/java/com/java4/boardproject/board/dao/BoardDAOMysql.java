...
public class BoardDAOMysql {
	...
	public List<Board> getAll(int start){
		return jdbcTemplate.query("select a.*, b.name as user_name, b.user_id as user_nick, b.git_address as user_git_address from boards a join users b on a.user_id=b.id order by a.id desc limit ?, 10", mapper, start);
